...
{
    ip.resize(Config::config->M);
    // init("127.0.0.1", 1234);
    for (int i = 0; i < Config::config->M; i++)
    {
        ip[i] = {"10.176.34.173"};
    }
    MatrixXu port(Config::config->M, Config::config->M);
    port.setZero();
    u64 p = 10001;
    for (int i = 0; i < Config::config->M; i++)
    {
    ...
    port(Config::config->M - 1, Config::config->M - 1) = 0;

    init(ip, port);
}

void SocketManager::pMPL::init(vector<string> &ip, MatrixXu &port)
{
    this->ip = ip;
    this->port = port;
    init_windows_socket();
...
void SocketManager::send(MatrixXu *a, int target)
{
...
void SocketManager::send(Matrix8 *a, int target)
{
    socket_io[party][target]->send_message(a);
}

void SocketManager::send(Matrixint128 *a, int target)
{
    socket_io[party][target]->send_message(a);
}

void SocketManager::receive(MatrixXu *a, int from)
{
    socket_io[party][from]->recv_message(*a);
}

void SocketManager::receive(Matrix8 *a, int from)
{
    socket_io[party][from]->recv_message(*a);
}

void SocketManager::receive(Matrixint128 *a, int from)
{
    socket_io[party][from]->recv_message(*a);
}
